...
@section('title')
{{ $title=trans('blog::messages.blog').' '.trans('blog::messages.post').' '.trans('blog::messages.new') }}
@stop
...
<div class="form-group hide">
  ...
  <div class="col-sm-4">
    <select class="form-control" name="is_comment">
      ...
      <option value="1" selected>{{trans('blog::messages.active')}}</option>
      <option value="-1">{{trans('blog::messages.deactive')}}</option>
    </select>
  </div>
</div>
...
<div class="tree well">
  ...
  {{\Modules\Blog\Models\Category::render([],'checkbox',false,false,false)}}
</div>
